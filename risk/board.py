...
class Board(object):
    ...
    def create(cls, n_players):
        """
        Create a Board and randomly allocate the territories. Place one
    ...
    def friendly_neighbors(self, territory_id):
        ...
        $Returns:
            generator: Generator of tuples of the form (territory_id,
            player_id, armies).
        """
        player_id = self.owner(territory_id)
        neighbor_ids = risk.definitions.territory_neighbors[territory_id]
    ...
    def is_valid_attack_path(self, path):
        ...
        All valid attacks, therefore, will follow a path of starting on one
        player's territory and moving trough enemy territories.

        Formally, an attack path is a valid path satisfying the following two
        additional properties:
        1. An attack path must contain at least two territories;
        1. If the first territory is owned by player A, then no other
        territories
        in the path are also owned by A.
        ...
        player_A = self.data[path[0]].player_id
        for territory_id in path[1:]:
            if self.data[territory_id].player_id == player_A:
                return False
    ...
    def cost_of_attack_path(self, path):
        """
        The cost of an attack path is the total number of enemy armies
        in the path.
        In other words, it is the total number of armies in the subpath
        starting at the second vertex.

        Args:
            path ([int]): a list of territory_ids which must be a valid attack
            path
        ...
        if not self.is_valid_attack_path(path):
            raise ValueError("Invalid attack path")

        cost = 0
        for territory_id in path[1:]:
            if self.owner(territory_id) != self.owner(path[0]):
                cost += self.armies(territory_id)

        return cost

    def shortest_path(self, source, target):
        """
        This function uses BFS to find the shortest path between source and
        target.
        This function does not take into account who owns the territories or
        how many armies are on the territories,
        ...
        while queue:
            ...
            if node not in visited:
                ...
                for neighbor in self.neighbors(node):
                    new_path = list(path)
                    new_path.append(neighbor.territory_id)
    ...
    def can_fortify(self, source, target):
        ...
        there must be a valid path between the source and target
        territories that is owned entirely by the same player.
        ...
        while queue:
            ...
            if node not in visited:
                visited.add(node)
                for neighbor in self.neighbors(node):
                    if self.owner(neighbor.territory_id) == source_owner:
                        queue.append(neighbor.territory_id)
        return False

    def cheapest_attack_path(self, source, target):
        """
        This function uses Dijkstra's algorithm to calculate a cheapest
        valid attack path between two territories if such a path exists.
        There may be multiple valid cheapest attack paths (in which case it
        doesn't matter which this function returns),
    ...
    def owns_continent(self, player_id, continent_id):
        ...
        """
        return all(
            (t.player_id == player_id for t in self.continent(continent_id))
        )
    ...
    def num_foreign_continent_territories(self, continent_id, player_id):
        """
        Compute the number of territories owned by other players on a given
        continent.
        ...
        Returns:
            int: The number of territories on the continent owned by other
            players.
        """
        return sum(
            1 if t.player_id != player_id else 0
    ...
    def fortify(self, from_territory, to_territory, n_armies):
        ...
        Raises:
            ...
            ValueError if the territories do not share a border or are not
            owned
            by the same player.
        """
        ...
        if to_territory not in [
            ...
            raise ValueError("Board: Cannot fortify, territories" +
                             "do not share owner and / or border.")
    ...
    def attack(self, from_territory, to_territory, attackers):
        ...
        Raises:
            ...
            ValueError if a player attacks himself or the territories do not
            share a border.

        Returns:
            bool: True if the defensive territory was conquered, False
            otherwise.
        """
        if attackers < 1 or self.armies(from_territory) <= attackers:
            raise ValueError("Board: Cannot attack with {n} armies from")

        if to_territory not in [
            tid for (tid, _, _) in self.hostile_neighbors(from_territory)
        ]:
            raise ValueError("Board: Cannot attack, territories do not share"
                             "border or are owned by the same player.")
    ...
    def plot_board(self, path=None, plot_graph=False, filename=None):
        ...
        Args:
            ...
            plot_graph (bool): if true, plots the graph structure overlayed
            on the board
            filename (str): if given, the plot will be saved to the given
            filename instead of displayed
        """
        ...
        def plot_path(xs):
            ...
            for i, x in enumerate(xs[1:]):
                if (xs[i] == 19 and xs[i + 1] == 1) or (
                    xs[i] == 1 and xs[i + 1] == 19
                ):
    ...
    def plot_single(territory_id, player_id, armies):
        ...
        plt.text(
            ...
            s=str(armies),
            color="black"
            if risk.definitions.player_colors[player_id] in ["yellow", "pink"]
            else "white",
            ha="center",
            size=15,
        )
    ...
    def fight(cls, attackers, defenders):
        ...
        Returns:
            tuple (int, int): Number of lost attackers, number of lost
            defenders.
        """
        ...
        wins = [
            att_d > def_d for att_d, def_d in zip(attack_dices, defend_dices)
        ]
        return len([w for w in wins if w is False]), len(
            [w for w in wins if w is True]
        )
    ...
    def set_armies(self, territory_id, n):
        ...
        """
        if n < 1:
            raise ValueError("Board: cannot set the number of armies to < 1"
                             "({tid}, {n}).".format(tid=territory_id, n=n))

        self.data[territory_id] = Territory(
            territory_id, self.owner(territory_id), n
        )
    ...
    def mobile(self, player_id):
        """
        Create a generator of all territories of a player which can attack or
        move,
        ...
        """
        return (
            t for t in self.data if (t.player_id == player_id and t.armies > 1)
        )
